...
const Index = () => {
  ...
  const lastWatchedChannel = currentProfile?.lastWatched && playlist?.channels 
    ? playlist.channels.find(ch => ch.id === currentProfile.lastWatched) 
    : null;
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="h-20 w-20 sm:h-24 sm:w-24 relative overflow-hidden rounded-full gold-glow">
            ...
            <div className="absolute inset-2 bg-steadystream-black rounded-full flex items-center justify-center">
              <div className="h-10 w-10 sm:h-12 sm:w-12 wave-pattern rounded-sm"></div>
            </div>
          </div>
        ...
        <div className="w-full max-w-5xl mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {profiles.map(profile => (
              <Card 
                key={profile.id} 
                className={`bg-black border-steadystream-gold/20 cursor-pointer hover:gold-glow transition-all duration-300 w-28 ${
                  currentProfile?.id === profile.id ? 'border-steadystream-gold border-2' : ''
                }`}
                onClick={() => {
                ...
                <CardContent className="flex flex-col items-center justify-center pt-6 pb-4">
                  <Avatar className="h-16 w-16 mb-2 border-2 border-steadystream-gold/30">
                    {profile.avatar ? (
                      ...
                      <AvatarFallback className="bg-steadystream-gold/20 text-steadystream-gold">
            ...
            <Card 
              className="bg-black border-steadystream-gold/20 border-dashed cursor-pointer hover:gold-glow transition-all duration-300 w-28"
            ...
            <CardContent className="flex items-center gap-4">
              <div className="w-20 h-20 bg-steadystream-black/50 rounded flex items-center justify-center overflow-hidden">
                {lastWatchedChannel.logo ? (
                  <img
                    src={lastWatchedChannel.logo}
                    alt={lastWatchedChannel.name}
              ...
              <div className="flex-1">
                ...
                {lastWatchedChannel.group && (
                  <p className="text-steadystream-secondary text-sm">{lastWatchedChannel.group}</p>
                )}
              </div>
            </CardContent>
            <CardFooter>
              <Button 
                ...
                onClick={() => navigate(`/player/${lastWatchedChannel.id}`)}
              >
                Continue Watching
              </Button>
            </CardFooter>
